...
import {
  CARD_SIZES,
  type CardFormat,
  type ScorecardArticleData,
  loadArticleCardFonts,
  drawArticleCard,
} from './canvasUtils';

export function ArticleScorecardButton({ data }: { data: ScorecardArticleData }) {
  ...
  const render = useCallback(() => {
    ...
    if (!ctx) return;
    drawArticleCard(ctx, width, height, data);
  }, [format, data]);
  ...
  function handleDownload() {
    canvasRef.current?.toBlob((blob) => {
      ...
      a.href = url;
      a.download = `paddockintel-article-${format}.png`;
      a.click();
  ...
  return (
    <>
      <button
        ...
        className="w-9 h-9 flex items-center justify-center border border-border-subtle text-text-1 hover:text-terracotta hover:border-terracotta transition-colors duration-150"
