...
export function ConstructorScorecardButton({ data }: { data: ScorecardConstructorData }) {
  ...
  const render = useCallback(() => {
    ...
    if (!ctx) return;
    drawConstructorCard(ctx, width, height, data, logoRef.current, teamColor);
  }, [format, data, teamColor]);
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (!logoRef.current) {
        logoRef.current = await loadLogoImage();
      }
      if (!cancelled) render();
    }
    init();
  ...
  function handleDownload() {
    canvasRef.current?.toBlob((blob) => {
      ...
      a.href = url;
      a.download = `paddockintel-${data.constructorRef}-${format}.png`;
      a.click();
