      ...
      <section className="soft-card mt-6 min-w-0 p-4 md:p-5" aria-label={contextLabel}>
        ...
        <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-3">
          {data.callouts.map((callout, index) => (
            <div key={`${index}-${callout.label}`} className="min-w-0 rounded-sm bg-surface-raised px-3 py-3">
  ...
  return (
    <section className="mt-3 min-w-0" aria-label={contextLabel}>
      ...
      <ul className="mt-1.5 flex flex-col gap-1.5">
        {data.callouts.map((callout, index) => (
          <li key={`${index}-${callout.label}`} className="min-w-0 rounded-sm bg-surface-raised px-2.5 py-1.5">
