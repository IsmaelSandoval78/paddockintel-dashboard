...
interface StandingsPanelProps {
  ...
  /** Narrow-column rendering — drivers stacked above constructors (never side by
   * side) and no outer soft-card/padding, for use inside a tile whose parent
              ...
              return (
                <div key={d.driver_id}>
                  <div className="flex items-center gap-3 mb-1.5">
                    ...
                    <span className="font-sans text-sm text-text-1 flex-1 min-w-0 truncate">{d.forename} {d.surname}</span>
                    <span className="font-mono text-[11px] text-text-3 shrink-0 hidden sm:inline">{d.constructor_name}</span>
                    <span className="flex flex-col items-end shrink-0">
                      <span className="font-sans font-bold tabular-nums text-base leading-none">{d.points}</span>
                      <span
                        className="font-mono text-[9px] tabular-nums tracking-[0.04em] mt-1"
                        style={{ color: d.position === 1 ? 'var(--text-2)' : 'var(--accent-2)' }}
                      >
              ...
              return (
                <div key={c.constructor_id}>
                  <div className="flex items-center gap-3 mb-1.5">
                    ...
                    <span className="font-sans text-sm text-text-1 flex-1 min-w-0 truncate">{c.name}</span>
                    <span className="flex flex-col items-end shrink-0">
                      <span className="font-sans font-bold tabular-nums text-base leading-none">{c.points}</span>
                      <span
                        className="font-mono text-[9px] tabular-nums tracking-[0.04em] mt-1"
                        style={{ color: c.position === 1 ? 'var(--text-2)' : 'var(--accent-2)' }}
                      >
