...
import {
  CARD_SIZES,
  type CardFormat,
  type ScorecardRecordData,
  loadRecordFonts,
  drawRecordCard,
} from '@/components/scorecards/canvasUtils';

export function RecordScorecardButton({ data, slug }: { data: ScorecardRecordData; slug: string }) {
  ...
  const render = useCallback(() => {
    ...
    if (!ctx) return;
    drawRecordCard(ctx, width, height, data);
  }, [format, data]);
  ...
  function handleDownload() {
    canvasRef.current?.toBlob((blob) => {
      ...
      a.href = url;
      a.download = `paddockintel-${slug}-${format}.png`;
      a.click();
