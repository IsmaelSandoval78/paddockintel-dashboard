...
- **Data-authority pipeline live 2026-07-06**: loader fixed (pit stops never extracted since it shipped — FastF1 in/out-lap semantics; lap_times + fastest laps now loaded too; rounds 7-9 backfilled), `scripts/data_desk.py` generates ranked post-race findings, and the first data autopsy shipped: `british-gp-2026-data-analysis` published in EN/ES/PT with hreflang + NewsArticle/BreadcrumbList/FAQPage schema, all figures query-backed. Weekly flow: load_race → data_desk → article.
...
- [x] **Full historical migration — DONE 2026-07-05.** Ghost(Pro) came back online (private mode) after the reactivation, so instead of the CSV manifest the content was pulled fresh via the Content API: **99 published posts** imported into `articles` via `scripts/import_ghost_articles.py` (the "106" figure included drafts/pages; 99 is what Ghost serves as published). Same slugs + published_at preserved → old URLs resolve 200 in production at the same root-level paths, verified on 3 spot-checks. Text-only by deliberate choice (no images imported — magazine is typographic/flat; Ghost CDN dependency eliminated). Kicker/H1/meta header block stripped from each body (new template renders title itself). Each article got its own `translation_group_id`; ES/PT translation is the remaining step (Batch API plan agreed, ~$10-15 for 99×2 translations)
