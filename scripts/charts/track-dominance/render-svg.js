...
const COLOR_A = '#00A99D'; // Mercedes teal (ANT)
const COLOR_B = '#2B3A4A'; // site navy — distinguishes teammate RUS from same-livery ANT
...
// limitInputPixels: false because the SVG's viewBox is in raw telemetry
// coordinate units (tens of thousands wide), not CSS px -- with no explicit
// width/height attribute on the <svg>, sharp/librsvg tries to rasterize at
// that intrinsic size before our resize() ever runs, tripping its
// decompression-bomb guard ("Input image exceeds pixel limit"). Safe to
// disable here: this SVG is generated by us, not untrusted input.
const pngWidth = 1200;
const svgAspect = vbH / vbW;
sharp(Buffer.from(trackSvg), { density: 300, limitInputPixels: false })
