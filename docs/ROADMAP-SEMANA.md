...
|---|---|---|
| 1. Cloudflare | **CORTE REAL COMPLETO 8-9 sep: hub/paddockintel.com/www los 3 sirviendo desde el Worker de Cloudflare, no Vercel.** Custom Domains creados en el dashboard real (reemplazando los CNAME/A viejos hacia Vercel), verificado con `dig`/`curl` reales: HSTS, redirect apex→www, trailing slash, i18n y datos reales de Supabase todos correctos post-corte. **Hallazgo crítico resuelto en el momento:** el Worker seguía con el deploy del 4 sep (Cloudflare no se re-despliega solo como Vercel) — rebuildeado y redeployado con `main` actual antes de cerrar el corte. Ver `docs/CLOUDFLARE-MIGRATION.md` | Falta la segunda mitad del paso 4: monitoreo activo la primera hora post-corte, todavía no cumplida. Pendiente no bloqueante: automatizar el redeploy a Cloudflare (hoy es 100% manual, a diferencia de Vercel) |
...
- Verificado antes de deploy: `tsc --noEmit` limpio, `eslint` limpio, `npm run dev` real levantado y las 3 locales (`/en/`, `/es/`, `/pt/`) responden 200 con el trigger de login renderizado correctamente en el HTML servido, sin errores en el log del servidor.

**Verificación con clic real en producción, 8-9 sep 2026 — confirmada por Ismael, Critique Gate 5/5:** login con Google funcionando de punta a punta, badge de iniciales visible y persistente tras reload, sesión cruzando `paddockintel.com` ↔ `hub.paddockintel.com` sin pedir re-login. Con esto, el paso 6 completo (backend del 6 sep + UI del 8 sep) queda cerrado de punta a punta — código y comportamiento real verificado, no solo implementación.

**Bug real encontrado en el camino y corregido — documentado para referencia futura, mismo patrón de fallo que el del 6 sep pero con una causa distinta:**

El primer intento de login real (8 sep) redirigió a `http://localhost:3000/?code=...` en vez de volver a `paddockintel.com`. Auditoría de código confirmó que `buildRedirectUrl()` en `AuthWidget.tsx` arma el `redirectTo` de forma dinámica vía `window.location.origin` (nunca hardcodeado, sin variable de entorno) — la causa no estaba en el código de la app. Causa real, confirmada por Ismael en el dashboard de Supabase (Authentication → URL Configuration):

- **`Site URL` seguía en el default `http://localhost:3000`**, sin actualizar — es el fallback que usa Supabase cuando el `redirect_to` real no matchea ninguna entrada de `Redirect URLs`.
- **`Redirect URLs` tenía el apex sin `www` (`https://paddockintel.com/api/auth/callback`) y el wildcard `https://*.paddockintel.com/api/auth/callback`** (agregado el 6 sep para cubrir `www` y subdominios futuros) — pero `paddockintel.com` redirige a `www.paddockintel.com` a nivel Vercel (dominio, no código de este repo), y la entrada explícita para `https://www.paddockintel.com/api/auth/callback` faltaba. El wildcard de subdominio no cubría ese caso con la certeza necesaria, y sin esa entrada exacta, Supabase caía al `Site URL` default.

**Corregido en el dashboard de Supabase (no en código):** `Site URL = https://www.paddockintel.com`, más la entrada explícita `https://www.paddockintel.com/api/auth/callback` agregada a `Redirect URLs`. Verificado con el clic real posterior — funcionó.

**Lección para la próxima vez que se toque cualquier flujo de redirect de auth:** `Site URL` y `Redirect URLs` son dos campos separados en Supabase, y `Site URL` no se actualiza solo cuando se corrige `Redirect URLs` — confirmar ambos explícitamente, no asumir que arreglar uno alcanza. Ya pasó dos veces (6 sep: `Redirect URLs` sin la variante `www`; 8 sep: `Site URL` sin actualizar del todo pese al wildcard).
