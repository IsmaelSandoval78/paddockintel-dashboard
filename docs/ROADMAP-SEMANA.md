...
| 7. Vertical de datos puros | **Primera métrica en vivo, 10 sep: Qualifying Pace Delta** (`/records/qualifying-pace-delta`, EN/ES/PT). Agregado de temporada 2026, view `driver_qualifying_pace` en Supabase, metodología pública inline (gate de `DATA-EXPERT.md`). Sanity-check pasado: líder del índice (Antonelli) coincide con el líder real del campeonato. Verificado en vivo en producción post-deploy | Ninguno bloqueante — pendiente no urgente: definir la 2ª métrica del vertical (Race Pace Index quedó descartado para v1 por los huecos reales de `lap_times`) |
